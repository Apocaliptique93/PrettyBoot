...
<div class="row">
<div class="span2">
    <h3>Edit server/API</h3>
    {{ Form::open() }}

    {{ Form::label('url', 'Server URL') }}
    {{ Form::text('url', $server->url, array('placeholder' => ('http://'.$_SERVER['SERVER_NAME'].'/api.php'))) }}

    {{ Form::label('host', 'Host parameter') }}
    {{ Form::text('host', $server->host, array('class' => 'input-small')) }}

    {{ Form::label('time', 'Time parameter') }}
    {{ Form::text('time', $server->time, array('class' => 'input-small')) }}

    {{ Form::label('port', 'Port parameter') }}
    {{ Form::text('port', $server->port, array('class' => 'input-small')) }}

    {{ Form::label('method', 'Method parameter') }}
    {{ Form::text('method', $server->method, array('class' => 'input-small')) }}

    {{ Form::label('custom', 'Custom parameter(s)') }}
    {{ Form::text('custom', $server->custom, array('placeholder' => 'apikey=123key321&user=John')) }}
        <br />

    {{ Form::button('Edit server/API', array('class' => 'btn btn-danger', 'style' => 'width:220px;')) }}
    {{ Form::close() }}
</div>
...
<div class="span8 offset1">
    ...
    <ol>
        ...
        <li>Check if the result link looks like it should, if so click the "Edit server/API" button</li>
    </ol>
